    ...
    return (
        <div className="mx-auto max-w-4xl py-10 pb-20">
            ...
            <div className="flex flex-col gap-8">
                ...
                {/* Text Parser Form - Now Full Width */}
                <div className="rounded-xl border border-white/10 bg-[#141414] p-8 shadow-xl flex flex-col gap-6">
                    <div>
                        <h2 className="text-xl font-bold flex items-center gap-2 mb-1"><FileText className="h-6 w-6 text-primary" /> Raw Scraper Logs</h2>
                        <p className="text-sm text-gray-400 font-medium">Paste the VM59 output containing `.mp4` & `.jpg` URLs. It will auto-match, build titles natively, and download images instantly.</p>
                    </div>
                    ...
                    <div className="flex gap-6">
                        <div className="flex-1 flex flex-col gap-2">
                            <label className="text-xs uppercase font-bold text-gray-400">Default Category</label>
                            <input 
                                type="text" 
                                value={category} 
                                onChange={(e) => setCategory(e.target.value)} 
                                className="w-full rounded-md border border-white/10 bg-[#1a1a1a] px-4 py-3 text-sm text-white focus:ring-2 focus:ring-primary outline-none" 
                                placeholder="e.g. Desi" 
                            />
                        </div>
                        <div className="flex-1 flex flex-col gap-2">
                            <label className="text-xs uppercase font-bold text-gray-400">Default Tags</label>
                            <input 
                                type="text" 
                                value={tags} 
                    ...
                    <button
                        ...
                        className="w-full mt-4 flex items-center justify-center gap-2 rounded-md bg-primary px-6 py-4 text-sm font-bold text-white hover:bg-primary/80 disabled:opacity-50 transition-colors"
                    >
                        {loading ? <Loader2 className="h-6 w-6 animate-spin" /> : <Send className="h-6 w-6" />}
                        {loading ? "Parsing & Downloading Images..." : "Process Text Logs"}
                    </button>
                </div>
                ...
                {/* CSV File Dropzone - Moved to bottom */}
                <div className="rounded-xl border border-white/10 bg-[#141414] p-6 shadow-xl flex flex-col gap-4 mx-auto w-full max-w-2xl">
                    ...
                    <div className="w-full h-32 mt-1 relative rounded-xl border-2 border-dashed border-white/20 bg-[#1a1a1a] p-6 flex flex-col items-center justify-center gap-2 hover:border-primary/50 transition-colors">
                        <input
                            type="file"
                            accept=".csv"
                        ...
                        <div className="text-center">
                            <p className="text-sm font-medium text-white">{file ? file.name : "Drop CSV file here"}</p>
                            <p className="text-xs text-gray-500 mt-1">{file ? `${(file.size / 1024).toFixed(1)} KB` : "Supports .csv up to 50MB"}</p>
                        </div>
                    </div>
